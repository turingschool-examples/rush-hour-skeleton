...
class RegisterAnApplication < ControllerTest
  ...
  def test_identifier_is_different_but_url_is_same
    ...
    assert_equal 200, last_response.status
    assert_equal "{'identifier':'jumpstartlab'}", last_response.body
    post '/sources', { identifier: "jumpstartlab2", root_url: "http://jumpstartlab.com"}
  ...
  def test_root_url_is_the_missing_parameter
  ...
  def test_root_url_is_an_empty_string
    post '/sources', { identifier: "jumpstartlab", root_url: "" }
    assert_equal 0, Site.count
    assert_equal 400, last_response.status
    assert_equal "Root url can't be blank", last_response.body
  end
