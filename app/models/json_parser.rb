...
class JsonParser
  def self.parse_json(payload, source)
    ...
    url = TrafficSpy::URL.create({:url => json_payload["url"]})
    event = TrafficSpy::Event.create({:event_name => json_payload["eventName"]})
    user_agent = TrafficSpy::UserAgent.create({:user_agent => json_payload["userAgent"]})
    ip = TrafficSpy::Ip.create({:ip => json_payload["ip"]})
